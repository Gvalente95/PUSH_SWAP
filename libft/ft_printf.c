...
/*   Updated: 2024/10/31 19:21:08 by gvalente         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	print_val_num(char frm, va_list args)
{
	char	*str;
	int		len;
...
int	print_val(char frm, va_list args)
{
	if (frm == 'd' || frm == 'i' || frm == 'u' || frm == 'x' || frm == 'X')
		return (print_val_num(frm, args));
	if (frm == 'c')
...
int	elems(const char *format, ...)
{
	...
	while (format[i])
	{
		...
		{
			cur_len = print_val(format[i + 1], args);
			if (cur_len == -1)
				return (-1);
			len += cur_len;
			format++;
